...
public class ModelMetrics {
    ...
    public static double[] computeARcorrelation(double[] dataArray, int p, int q){
        double[][] toplizeArray=new double[p][p];

        double[] autocorr = autocorData(dataArray,p+q);
        double[] autocorrF = computeAutoCorrelation(dataArray, p+q);
        for(int i=1;i<=p;i++)
        {
            int k=1;
            for(int j=i-1;j>0;j--)
            {
                toplizeArray[i-1][j-1]=autocorr[q+k++];
            }
            toplizeArray[i-1][i-1]=autocorr[q];
            int kk=1;
            for(int j=i;j<p;j++)
            {
                toplizeArray[i-1][j]=autocorr[q+kk++];
            }
        }
    ...
    public static double[] computeAutoCorrelation(double[] dataArray, int order)
    {
    ...
    {
        double[] autoCor=new double[order+1];
        double varData=varerrData(dataArray);
    ...
    public static double[] computeMApara(double[] autocorData,int q){
